...
const DailyReportComponent: React.FC<DailyReportProps> = ({ patients }) => {
  ...
  const renderSpecialtyView = () => (
    <>
      {specialties.map((specialty) => {
        ...
        const activePatients = specialtyPatients.filter(p => p.status === 'Active');
        const dischargedPatients = specialtyPatients.filter(p => p.status === 'Discharged');
              ...
              <div>
                ...
                <ul className="space-y-2">
                  {activePatients.map((patient) => (
                    <li key={`active-${patient.id}`} className="bg-green-50 p-2 rounded-md text-green-800">
              ...
              <div>
                ...
                <ul className="space-y-2">
                  {dischargedPatients.map((patient) => (
                    <li key={`discharged-${patient.id}`} className="bg-gray-50 p-2 rounded-md text-gray-800">
